...
export const DEFAULT_OPENCODE_OUTPUT_INACTIVITY_TIMEOUT_MS = 7 * 60 * 1000;
...
export interface OpenCodeOutputInactivityMonitorOptions {
   ...
   * JSON via the OpenCode JSONL parser counts as a heartbeat event.
   */
 ...
 * `monitor: no opencode output for {N}m {S}s` shape.
 */
export function formatOutputInactivityMonitorErrorMessage(elapsedMs: number): string {
  ...
  return `monitor: no opencode output for ${minutes}m ${seconds}s`;
}
